package fr.rouen.univ.mining.algorithms.frequentpatterns.apriori_close;
...
import fr.rouen.univ.mining.algorithms.ArraysAlgos;
import fr.rouen.univ.mining.patterns.itemset_array_integers_with_count.Itemset;
import fr.rouen.univ.mining.patterns.itemset_array_integers_with_count.Itemsets;
import fr.rouen.univ.mining.tools.MemoryLogger;
...
public class AlgoAprioriClose {
    ...
    private HashMap<String, Integer> myCustomIdMapThatRocks;
    private HashMap<Integer, String> reverseCustomIdMapThatRocks;
    ...
    public AlgoAprioriClose() {
        myCustomIdMapThatRocks = new HashMap<>();
        reverseCustomIdMapThatRocks = new HashMap<>();
    }
    ...
    public Itemsets runAlgorithm(double minsup, String input, String output) throws IOException {
        ...
        while (((line = reader.readLine()) != null)) { 
            ...
            for (int i=0; i< lineSplited.length; i++) { 
                ...
                if (! myCustomIdMapThatRocks.containsKey(lineSplited[i])) {
                    myCustomIdMapThatRocks.put(lineSplited[i], myCustomIdMapThatRocks.size());
                    reverseCustomIdMapThatRocks.put(myCustomIdMapThatRocks.size(), lineSplited[i]);
                }

                Integer item = myCustomIdMapThatRocks.get(lineSplited[i]);
...
void saveItemset(Itemset itemset) throws IOException {
     ...
     void saveItemset(Integer item, Integer support) throws IOException {
        ...
        // if the result should be saved to a file
        if(writer != null){
            writer.write(reverseCustomIdMapThatRocks.get(item) + " #SUP: " + support);
            writer.newLine();
        }// otherwise the result is kept into memory
        else{
